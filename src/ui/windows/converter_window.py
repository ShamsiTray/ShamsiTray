...
class DateConverterWindow(BaseFramelessWindow):
    ...
    def _setup_ui(self):
        ...
        self.middle_divider.hide(); self.output_widget.hide(); self.status_message.hide()

    def _create_title_bar(self) -> QHBoxLayout:
        self.exit_button = QPushButton("✕"); self.minimize_button = QPushButton("—")
    ...
    def _create_input_section(self) -> QHBoxLayout:
        grid = QGridLayout(); grid.setHorizontalSpacing(15); grid.setVerticalSpacing(5)
        labels = {"نوع تبدیل": (0, 0), "روز": (0, 1), "ماه": (0, 2), "سال": (0, 3)}
        self.input_labels = []
        for text, pos in labels.items():
            label = QLabel(text); label.setAlignment(Qt.AlignCenter)
            grid.addWidget(label, pos[0], pos[1]); self.input_labels.append(label)

        self.conversion_type_combo = QComboBox()
        self.conversion_type_combo.addItems(["شمسی به میلادی", "میلادی به شمسی"])
        self.day_combo = QComboBox(); self.month_combo = QComboBox()
        for combo in [self.conversion_type_combo, self.day_combo, self.month_combo]:
            combo.setEditable(True); combo.lineEdit().setReadOnly(True)
            combo.lineEdit().setAlignment(Qt.AlignCenter); combo.lineEdit().installEventFilter(self)

        self.year_input = QLineEdit(); self.year_input.setAlignment(Qt.AlignCenter)
        self.year_validator = QIntValidator(1, 9999, self); self.year_input.setValidator(self.year_validator)
        grid.addWidget(self.conversion_type_combo, 1, 0); grid.addWidget(self.day_combo, 1, 1)
        grid.addWidget(self.month_combo, 1, 2); grid.addWidget(self.year_input, 1, 3)
        wrapper_layout = QHBoxLayout(); wrapper_layout.addStretch(1)
        wrapper_layout.addLayout(grid); wrapper_layout.addStretch(1)
        return wrapper_layout

    def _create_output_section(self) -> QWidget:
        widget = QWidget(); layout = QVBoxLayout(widget)
        layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(10)
        output_grid = QHBoxLayout(); output_grid.setSpacing(10); output_grid.setAlignment(Qt.AlignCenter)
        ...
        layout.addLayout(output_grid); layout.addSpacing(10)
        self.elapsed_text = QLabel("..."); self.elapsed_text.setAlignment(Qt.AlignCenter)
        self.leap_year_status = QLabel("..."); self.leap_year_status.setAlignment(Qt.AlignCenter)
        ...
        layout.addWidget(self.elapsed_text); layout.addWidget(self.leap_year_status)
        return widget

    def _create_output_group(self, title: str) -> Tuple[QLabel, QLabel, QVBoxLayout]:
        layout = QVBoxLayout(); layout.setAlignment(Qt.AlignCenter); layout.setSpacing(0)
        title_label = QLabel(title); title_label.setObjectName("OutputTitleLabel"); title_label.setAlignment(Qt.AlignCenter)
        value_label_1 = QLabel("..."); value_label_2 = QLabel("...")
        for label in [value_label_1, value_label_2]: label.setAlignment(Qt.AlignCenter); label.setWordWrap(True)
        layout.addWidget(title_label); layout.addWidget(value_label_1); layout.addWidget(value_label_2)
    ...
    def update_styles(self):
        palette = self.palette(); p = APP_CONFIG.get_current_palette()
        text_color = p['TEXT_COLOR']; palette.setColor(QPalette.WindowText, QColor(text_color))
        palette.setColor(QPalette.Text, QColor(text_color)); self.setPalette(palette)
        for label in self.findChildren(QLabel):
            if not label.objectName(): label.setStyleSheet(f"color: {text_color};")
        
        input_field_height = 40; input_bg = p['INPUT_BG_COLOR']; border_color = p['CALENDAR_BORDER_COLOR']; accent_color = p['ACCENT_COLOR']
        ...
        self.exit_button.setFixedSize(30, 30); self.exit_button.setStyleSheet(f"QPushButton {{ background-color: #FF4C4C; {base_button_style} }} QPushButton:hover {{ background-color: #FF6666; }}")
        self.minimize_button.setFixedSize(30, 30); self.minimize_button.setStyleSheet(f"QPushButton {{ background-color: {p['BACKGROUND_COLOR']}; {base_button_style} }} QPushButton:hover {{ background-color: {p['HOVER_BG']}; }}")
        for label in self.input_labels: label.setFont(QFont(APP_CONFIG.FONT_FAMILY, 14)); label.setFixedHeight(input_field_height)
        self.conversion_type_combo.setFixedSize(220, input_field_height); self.day_combo.setFixedSize(80, input_field_height); self.month_combo.setFixedSize(140, input_field_height)
        for combo in [self.conversion_type_combo, self.day_combo, self.month_combo]: combo.setStyleSheet(combo_style)
        ...
        self.convert_button.setFixedHeight(50); self.convert_button.setStyleSheet(f"QPushButton {{ background-color: {accent_color}; color: white; border: none; border-radius: 8px; font-size: 16px; font-weight: bold; font-family: '{APP_CONFIG.FONT_FAMILY}'; }} QPushButton:hover {{ background-color: {QColor(accent_color).darker(120).name()}; }}")
        self.middle_divider.setStyleSheet(f"color: {border_color};")
        for label in self.output_widget.findChildren(QLabel):
            if label.objectName() == "OutputTitleLabel": label.setFont(QFont(APP_CONFIG.FONT_FAMILY, 16, QFont.Bold)); label.setStyleSheet(f"color: {accent_color}; padding: 5px;")
        self.jalali_date_value_1.setFont(QFont(APP_CONFIG.FONT_FAMILY, 15, QFont.Bold)); self.jalali_date_value_2.setFont(QFont(APP_CONFIG.FONT_FAMILY, 14))
        self.gregorian_date_value_1.setFont(QFont(APP_CONFIG.GREGORIAN_FONT_FAMILY, 15, QFont.Bold)); self.gregorian_date_value_2.setFont(QFont(APP_CONFIG.GREGORIAN_FONT_FAMILY, 14))
        self.elapsed_text.setFont(QFont(APP_CONFIG.FONT_FAMILY, 14)); self.leap_year_status.setFont(QFont(APP_CONFIG.FONT_FAMILY, 12))
        self.status_message.setStyleSheet("color: red; font-weight: bold; font-size: 18px;")

    def _connect_signals(self):
        self.exit_button.clicked.connect(self.close); self.minimize_button.clicked.connect(self.showMinimized)
        ...
        self.convert_button.clicked.connect(self._convert_date); self.year_input.returnPressed.connect(self._convert_date)

    def _apply_windows_styles(self):
        if sys.platform == "win32" and WIN32_AVAILABLE:
            try:
                hwnd = self.winId().__int__(); style = win32gui.GetWindowLong(hwnd, win32con.GWL_STYLE)
                style |= win32con.WS_MINIMIZEBOX | win32con.WS_SYSMENU; win32gui.SetWindowLong(hwnd, win32con.GWL_STYLE, style)
            ...
            except Exception as e: logger.error(f"Failed to apply Windows window styles: {e}")
    ...
    def _on_conversion_type_changed(self):
        ...
        if is_jalali_to_greg:
            self.year_input.setText(to_persian_digits(str(today.year))); self.year_validator.setRange(self.MIN_JALALI_YEAR, self.MAX_JALALI_YEAR)
            months = [f"{to_persian_digits(i+1)} - {name}" for i, name in enumerate(APP_CONFIG.PERSIAN_MONTHS)]
            self.month_combo.view().setLayoutDirection(Qt.RightToLeft); self.day_combo.view().setLayoutDirection(Qt.RightToLeft)
        else:
            self.year_input.setText(str(today.year)); self.year_validator.setRange(self.MIN_GREGORIAN_YEAR, self.MAX_GREGORIAN_YEAR)
            months = [f"{i+1:02d} - {name}" for i, name in enumerate(APP_CONFIG.GREGORIAN_MONTHS)]
            self.month_combo.view().setLayoutDirection(Qt.LeftToRight); self.day_combo.view().setLayoutDirection(Qt.LeftToRight)
        
        self.month_combo.clear(); self.month_combo.addItems(months); self.month_combo.setCurrentIndex(today.month - 1)
        self._update_day_dropdown()
        day_str_to_set = to_persian_digits(today.day) if is_jalali_to_greg else str(today.day)
        if self.day_combo.findText(day_str_to_set) != -1: self.day_combo.setCurrentText(day_str_to_set)
    ...
    def _update_day_dropdown(self, *args):
        ...
        try:
            year_str = self.year_input.text(); year = int(from_persian_digits(year_str)) if year_str else 1
            month = self.month_combo.currentIndex() + 1; days_in_month = self._get_days_in_month(is_jalali_input, year, month)
        except (ValueError, IndexError): days_in_month = 31 
        
        self.day_combo.blockSignals(True); self.day_combo.clear()
        days = [to_persian_digits(d) if is_jalali_input else str(d) for d in range(1, days_in_month + 1)]; self.day_combo.addItems(days)
        if self.day_combo.findText(current_day_text) != -1: self.day_combo.setCurrentText(current_day_text)
        elif self.day_combo.count() > 0: self.day_combo.setCurrentIndex(self.day_combo.count() - 1)
        self.day_combo.blockSignals(False)

    def _get_days_in_month(self, is_jalali: bool, year: int, month: int) -> int:
        if is_jalali:
            if 1 <= month <= 6: return 31;
            if 7 <= month <= 11: return 30;
            if month == 12: return 30 if is_jalali_leap_year(year) else 29
        else: return (datetime.date(year, month + 1, 1) - datetime.timedelta(days=1)).day if month < 12 else 31
        return 0
    ...
    def _show_error(self, message: str):
        self._clear_results(); self.status_message.setText(message); self.status_message.show()
    ...
    def _convert_date(self):
        ...
        is_jalali_input = self.conversion_type_combo.currentText() == "شمسی به میلادی"
        try:
            year_text = self.year_input.text(); day_text = self.day_combo.currentText()
            if not year_text or not day_text: raise ValueError("Year or day is empty.")
            year = int(from_persian_digits(year_text)); month = self.month_combo.currentIndex() + 1; day = int(from_persian_digits(day_text))
            if is_jalali_input: j_date = jdatetime.date(year, month, day); g_date = j_date.togregorian()
            else: g_date = datetime.date(year, month, day); j_date = jdatetime.date.fromgregorian(date=g_date)
            self._display_results(j_date, g_date, is_jalali_input)
        except (ValueError, TypeError) as e: self._show_error("تاریخ ورودی نامعتبر است."); logger.warning(f"Date conversion failed: {e}")
    ...
    def _display_results(self, j_date: jdatetime.date, g_date: datetime.date, is_jalali_input: bool):
        ...
        self.middle_divider.show(); self.output_widget.show(); self.setFixedSize(650, 550)

    def _update_elapsed_time(self, target_date: datetime.date):
        today = datetime.date.today()
        if target_date == today: self.elapsed_text.setText("تاریخ امروز است."); return
        ...
        if RELATIVEDELTA_AVAILABLE:
            ...
            years, rem = divmod(total_days, 365); months, days = divmod(rem, 30)
